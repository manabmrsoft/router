...
use DI\Container;
use Leo\Exceptions\InvalidArgumentException;
use Leo\Http\Exceptions\MalformedUrlException;
use Leo\Http\Middleware\BaseCsrfVerifier;
use Leo\Http\Request;
use Leo\Http\Response;
use Leo\Http\Url;
use Leo\SimpleRouter\ClassLoader\IClassLoader;
use Leo\SimpleRouter\Exceptions\HttpException;
use Leo\SimpleRouter\Handlers\CallbackExceptionHandler;
use Leo\SimpleRouter\Handlers\IEventHandler;
use Leo\SimpleRouter\Route\IGroupRoute;
use Leo\SimpleRouter\Route\IPartialGroupRoute;
use Leo\SimpleRouter\Route\IRoute;
use Leo\SimpleRouter\Route\RouteController;
use Leo\SimpleRouter\Route\RouteGroup;
use Leo\SimpleRouter\Route\RoutePartialGroup;
use Leo\SimpleRouter\Route\RouteResource;
...
class SimpleRouter
{
     ...
     * Start routing
     *
     * @throws \Leo\SimpleRouter\Exceptions\NotFoundHttpException
     * @throws \Leo\Http\Middleware\Exceptions\TokenMismatchException
        ...
        if (is_file($composerFile) === true) {
            ...
            if (isset($composerInfo['packages']) === true && \is_array($composerInfo['packages']) === true) {
                foreach ($composerInfo['packages'] as $package) {
                    if (isset($package['name']) === true && strtolower($package['name']) === 'Leo/simple-router') {
